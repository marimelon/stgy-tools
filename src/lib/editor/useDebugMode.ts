...
export function setDebugMode(enabled: boolean): void {
  ...
  window.dispatchEvent(new CustomEvent(DEBUG_MODE_CHANGE_EVENT, { detail: enabled }));
}
...
export function useDebugMode() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(DEBUG_MODE_CHANGE_EVENT, handleDebugModeChange as EventListener);

    return () => {
      window.removeEventListener(DEBUG_MODE_CHANGE_EVENT, handleDebugModeChange as EventListener);
    };
  }, []);
