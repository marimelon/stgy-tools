    ...
    switch (fieldId) {
      ...
      case 4: {
        ...
        for (let i = 0; i < count; i++) {
          const flagValue = data[offset + 6 + i * 2] | (data[offset + 6 + i * 2 + 1] << 8);
      ...
      case 5: {
        ...
        const positions = [];
        for (let i = 0; i < count; i++) {
          const x = (data[offset + 6 + i * 4] | (data[offset + 6 + i * 4 + 1] << 8)) / 10;
          const y = (data[offset + 6 + i * 4 + 2] | (data[offset + 6 + i * 4 + 3] << 8)) / 10;
          positions.push({ x, y });
        }
      ...
      case 6: {
        ...
        const rotations = [];
        for (let i = 0; i < count; i++) {
          const val = data[offset + 6 + i * 2] | (data[offset + 6 + i * 2 + 1] << 8);
      ...
      case 12: {
        ...
        const params = [];
        for (let i = 0; i < count; i++) {
          params.push(data[offset + 6 + i * 2] | (data[offset + 6 + i * 2 + 1] << 8));
        }
        parsedValue = params;
...
export function compareStgy(
  ...
  try {
    ...
    binaryMatch = origBinary.length === reEncBinary.length &&
